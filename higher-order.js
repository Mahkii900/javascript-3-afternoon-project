...
//Code Here
let evenNumbers = mixedNumbers.filter(function(num){return !(num % 2)})
...
//Code Here
let postTaxPrices= prices.map(function(num){return num * 1.07});
...
//Code Here
let totalPopulation= populations.reduce(function(total, num){return total + num})
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(monster, cp){return monster.CP > 200})
...
let orderTotals = orders.map(function(item){return (item.price * (1 + item.tax))})
...
*/
let newBobs = purchases.map(function(purchase){return purchase.owner == "Bob" ? purchase.price : null})
let bobsTotal = newBobs.reduce(function(total, num){return total + num})